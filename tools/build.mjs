...
const origin = 'https://uniqoengine.github.io';
const esc = (s = '') => String(s).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
const groups = [
  ['Start here', [['getting-started', 'Getting started'], ['projects', 'Projects'], ['editor', 'The editor'], ['settings', 'Settings']]],
  ...
  ['Render & animate', [['lighting', 'Lighting & shadows'], ['sprites-particles', 'Sprites & particles'], ['environment-effects', 'Environment effects'], ['palette-animation', 'Palette animation']]],
  ['Build gameplay', [['scripting', 'C++ scripting'], ['input-collision', 'Input & collision'], ['runtime-services', 'Runtime services'], ['camera-resources', 'Cameras'], ['resources', 'Shared resources'], ['memory-card', 'Memory Card']]],
...
const special = { runtime: 'runtime/README.md', demo: 'examples/rpg-2-5d-demo/README.md', license: 'LICENSE', credits: 'THIRD_PARTY_NOTICES.md', 'runtime-credits': 'runtime/THIRD_PARTY_NOTICES.md' };
...
const features = [
  ...
  ['03', 'Write native gameplay', 'C++20 behaviours with properties in the Inspector. Input, collision, cameras, scene transitions and persistence.', 'scripting'],
...
const home = `<main id="main"><section class="hero wrap"><div class="hero-top"><div><p class="eyebrow"><span class="status-dot"></span> ORIGINAL PLAYSTATION DEVELOPMENT</p><h1>Build worlds.<br><span>Make them PSX.</span></h1></div><div class="hero-intro"><p>A standalone visual editor.<br>A native PlayStation runtime.<br>Your next game starts here.</p><div class="actions"><a class="button primary" href="/docs/getting-started/">Get started <span aria-hidden="true">→</span></a><a class="text-link" href="/docs/">Explore the docs ${arrow}</a></div><p class="small muted">Windows x64 <span>·</span> Open source <span>·</span> Experimental</p></div></div><figure class="editor-shot"><div class="window-bar"><span class="window-dots" aria-hidden="true">● ● ●</span><span>UniQo Editor <span class="muted">/ 2.5D courtyard</span></span><span class="live-label">REAL EDITOR CAPTURE</span></div><img src="/media/docs/images/uniqo-editor.png" width="1440" height="900" alt="UniQo editor showing the 2.5D courtyard, scene hierarchy and component inspector" fetchpriority="high"><figcaption><span>Build the scene. Write the behaviour. Press Play.</span><span class="mono">Rust + Dear ImGui / C++20 + PsyQo</span></figcaption></figure></section><section id="features" class="section wrap"><div class="section-heading"><div><p class="eyebrow">THE WORKFLOW</p><h2>From an empty scene<br>to a world of your own.</h2></div><p>Author visually, work within PSX limits,<br>and run native MIPS code.</p></div><div class="features">${features.map(([n, title, text, slug]) => `<a class="feature" href="/docs/${slug}/"><span class="feature-number mono">${n}</span><h3>${title} ${arrow}</h3><p>${text}</p></a>`).join('')}</div></section><section class="showcase section wrap"><figure><img src="/media/docs/images/forest-dialogue.png" width="640" height="480" alt="Native PSX output with a sprite character in a textured forest and portrait dialogue HUD" loading="lazy"><figcaption>Native 640 × 480 output in PCSX-Redux. AI-generated artwork prepared for PSX textures.</figcaption></figure><div><p class="eyebrow">MADE WITH UNIQO</p><h2>A small console.<br>A world to explore.</h2><p>Combine textured environments, animated sprites and dialogue UI. This scene runs as native PlayStation code in the emulator.</p><a class="text-link" href="/docs/demo/">Explore the included 2.5D demo ${arrow}</a><div class="tags"><a href="/docs/textures/">Textures</a><a href="/docs/sprites-particles/">Sprites</a><a href="/docs/hud/">HUD</a></div></div></section><section class="section getting-started wrap"><div><p class="eyebrow">YOUR FIRST GAME</p><h2>Clone. Create. Play.</h2><p>Install the tools, open the Sample game template and bring your first scene to life.</p><a class="button primary" href="/docs/getting-started/">Installation guide <span aria-hidden="true">→</span></a></div><div class="terminal"><div class="window-bar"><span>PowerShell</span><span class="muted">Windows x64</span></div><pre><code>git clone https://github.com/franadoriv/UniQo.git
cd UniQo
powershell -ExecutionPolicy Bypass -File tools/setup.ps1
cargo run --locked</code></pre><p>Requires Git, Rust and Visual Studio C++ Build Tools.<br>Use a checkout path without spaces.</p></div></section><section class="status-note wrap"><span class="status-dot"></span><div><h3>Built in the open. Still evolving.</h3><p>UniQo is experimental. Builds and emulator execution are validated; physical-console validation is pending. Original code is MIT licensed. <a href="/docs/#limits">Read the current limits →</a></p></div></section></main>`;
write('index.html', page('UniQo — Build games for the original PlayStation', 'A standalone visual editor and native PSX runtime. Build worlds, animate characters and write C++ gameplay for the original PlayStation.', '/', home));
const searchItems = [];
for (let i = 0; i < docs.length; i++) {
  ...
  write(`docs/${doc.slug}/index.html`, page(`${doc.label} — UniQo Docs`, `${doc.label}: guides and reference for the UniQo PlayStation game engine.`, `/docs/${doc.slug}/`, body, 'docs'));
}
...
const index = `<main id="main" class="wrap docs-index"><p class="eyebrow">UNIQO DOCUMENTATION</p><h1>Build your first world.<br><span>Then go deeper.</span></h1><p class="lead">Setup, workflows and native APIs for the original PlayStation.</p><form class="search-form" role="search" action="/docs/"><label for="search">Search the documentation</label><div class="search-box"><span aria-hidden="true">⌕</span><input id="search" name="q" type="search" placeholder="Try textures, collision or C++…" autocomplete="off"><button type="reset">Clear</button></div></form><p class="search-status small muted" role="status" aria-live="polite"></p><div class="search-results" hidden></div><div class="guide-grid">${docGroups}</div><section id="limits" class="limits"><p class="eyebrow">BEFORE YOU BUILD</p><h2>Know the current limits.</h2><ul><li><strong>Experimental, Windows x64.</strong> Builds and emulator execution are verified. Physical-console validation is pending.</li><li><strong>Rigid skeletal animation.</strong> Skeletal textures, blended skin weights and animation blending remain future work.</li><li><strong>Focused editing tools.</strong> Blockout and MCP scene batches support Undo/Redo. General editor Undo/Redo and entity multiselection remain future work.</li><li><strong>PSX rendering constraints.</strong> Intersecting polygons can still produce sorting artifacts. Capacity limits are not frame-rate guarantees.</li><li><strong>Bounded simulation and resources.</strong> Fixed 60 Hz steps, conservative AABB collision and resident scene data that must fit PSX RAM. Arbitrary map streaming from CD is not implemented in this documented revision.</li><li><strong>Build from source.</strong> A release build creates the editor executable; portable application packaging is not yet provided.</li></ul><p class="small muted">Documentation snapshot: <a href="${repo}/tree/${manifest.commit}">${manifest.commit.slice(0, 7)}</a>. Updated from committed engine documentation.</p></section></main>`;
write('docs/index.html', page('Documentation — UniQo', 'Learn UniQo: installation, editor workflows, assets, C++ scripting and the native PlayStation runtime.', '/docs/', index, 'docs'));
write('assets/search.json', JSON.stringify(searchItems));
write('404.html', page('Page not found — UniQo', 'Find your way back to the UniQo documentation.', '/404.html', '<main id="main" class="wrap not-found"><p class="eyebrow">404 / OUTSIDE THE SCENE</p><h1>This page is missing.</h1><p>The guide may have moved. Find it in the documentation.</p><a class="button primary" href="/docs/">Browse the docs →</a></main>'));
write('.nojekyll', '');

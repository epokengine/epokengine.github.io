...
3. Choose a `.epokasset` destination, **SFX** or **BGM / XA**, sample rate, trimming, normalization and whole-clip looping. BGM also offers mono/stereo. Conversion runs in the background; a validated package is published on success.
...
The package starts with `EPOKAS01`, two little-endian 32-bit lengths, JSON metadata and source bytes. Package/importer versions and the SHA-256 source checksum are validated. Derived cache keys include source content, settings, importer version and target format. Corrupt conversions regenerate.

...
Timer 0 requests approximately 1 kHz service, Timer 2 measures elapsed CPU/8 time, and the existing GPU Timer 1 provides a wrap guard. Timing is independent of rendered frames. A gap too long to measure unambiguously stops sequences with a clock fault. Legacy portable banks retain their linear software ADSR. SoundFont-derived EPSB v2 banks use bounded software envelopes/modulation, fixed-point gain/pan/tuning, prepared note states and shared optional PSX Room reverb. Arbitrary native Sony synthesis is not claimed. `epok::music_sequence_stats` exposes starts, loops, physical steals, priority-denied notes, capacity errors, peak voices, active instances, pitch clamps, clock faults, maximum service gap and CPU/8 service ticks. The diagnostic name changed from `epok::sequence_stats` to avoid collision with Timeline's existing counters, which keep that name. Read audio counters under `epok::SequenceLock` for a consistent snapshot. Per-song logical voice steals are also retained by the kernel. Physical hardware validation remains pending.
